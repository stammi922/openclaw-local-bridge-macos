...
step 8 $TOTAL "Patch proxy subprocess timeout"
node "$REPO_ROOT/scripts/patch-proxy-timeout.mjs" "$PROXY_HOME" $([[ $DRY_RUN -eq 1 ]] && echo --dry-run)

step 9 $TOTAL "Patch proxy subprocess manager (silence per-chunk debug logging)"
node "$REPO_ROOT/scripts/patch-manager-silent-debug.mjs" "$PROXY_HOME" $([[ $DRY_RUN -eq 1 ]] && echo --dry-run)

step 10 $TOTAL "Patch proxy subprocess manager (strip embedded NUL bytes from prompts)"
node "$REPO_ROOT/scripts/patch-manager-strip-null-bytes.mjs" "$PROXY_HOME" $([[ $DRY_RUN -eq 1 ]] && echo --dry-run)

step 11 $TOTAL "Patch proxy system-prompt isolation"
node "$REPO_ROOT/scripts/patch-proxy-system-prompt.mjs" "$PROXY_HOME" $([[ $DRY_RUN -eq 1 ]] && echo --dry-run)

step 12 $TOTAL "Patch proxy routes (bounded concurrency cap)"
node "$REPO_ROOT/scripts/patch-routes-concurrency-cap.mjs" "$PROXY_HOME" $([[ $DRY_RUN -eq 1 ]] && echo --dry-run)

step 13 $TOTAL "Patch proxy routes (per-sessionId serialization)"
node "$REPO_ROOT/scripts/patch-routes-session-serialize.mjs" "$PROXY_HOME" $([[ $DRY_RUN -eq 1 ]] && echo --dry-run)

step 14 $TOTAL "Patch proxy routes (keep-alive + empty-result fallback)"
node "$REPO_ROOT/scripts/patch-routes-stream-safety.mjs" "$PROXY_HOME" $([[ $DRY_RUN -eq 1 ]] && echo --dry-run)
...
step 17 $TOTAL "Patch proxy subprocess idle-timeout (resets on stream chunk, ~40min)"
node "$REPO_ROOT/scripts/patch-proxy-idle-timeout.mjs" --root "$PROXY_HOME" $([[ $DRY_RUN -eq 1 ]] && echo --dry-run)

step 18 $TOTAL "Patch proxy media passthrough (forward images/files to the CLI)"
node "$REPO_ROOT/scripts/patch-proxy-media-passthrough.mjs" "$PROXY_HOME" $([[ $DRY_RUN -eq 1 ]] && echo --dry-run)

step 19 $TOTAL "Install superpowers skills"
node "$REPO_ROOT/scripts/install-skills.mjs" $([[ $DRY_RUN -eq 1 ]] && echo --dry-run)

step 20 $TOTAL "Scaffold rotator bridge state and link CLI"
...
step 21 $TOTAL "Patch ~/.openclaw/openclaw.json"
...
step 22 $TOTAL "Patch gateway plist (if present)"
node "$REPO_ROOT/scripts/patch-gateway-plist.mjs" "$GATEWAY_PLIST" $([[ $DRY_RUN -eq 1 ]] && echo --dry-run)

step 23 $TOTAL "Render proxy plist & (re)load launchd services"
if (( DRY_RUN )); then
...
step 24 $TOTAL "Claude Code permissions"
add_claude_perms() {
  if (( DRY_RUN )); then
...
step 26 $TOTAL "Install MCP bridge binaries (openclaw-core-mcp, openclaw-watch)"
if (( DRY_RUN )); then
