...
| 18 | `patch-proxy-media-passthrough.mjs` | adapter + manager + routes | `@openclaw-bridge:media-passthrough v1` | **(2026-06-17)** Forward image/file attachments to the CLI instead of dropping them. Adapter gains `extractMediaBlocks()` (OpenAI `image_url`/`file` data-URLs → Anthropic `image`/`document` blocks); when present, manager switches argv to `--input-format stream-json` (drops the positional text prompt) and writes a `{type:"user",message:{role:"user",content:[text,...blocks]}}` envelope to the CLI's stdin; routes thread `cliInput.mediaBlocks` into both `subprocess.start()` sites. Depends on step 11 (anchors reference its `_nonSystem` binding). Fixes inbound Mattermost/webchat images reaching the model. |
