...
export const sessionsSendTool = {
  definition: {
    name: "sessions_send",
    description:
      "Send a message to an existing session by `session_id`. Waits up to `wait_ms` (default 15s); if the session closes within that window, returns status=done with its exit_code. Otherwise the session keeps running and this returns status=running — poll with session_status or watch via openclaw-watch. The final model reply is not returned inline; read the session log separately if needed.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  async handler(rawArgs: unknown): Promise<SessionsSendResult> {
    ...
    return { session_id, status: "done" as const, exit_code: winner };
  },
};
